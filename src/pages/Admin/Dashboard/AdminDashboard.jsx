...
const AdminDashboard = () => {
    ...
    const downloadFullReport = async () => {
        try {
            ...
            const financeRes = await fetch(`${API_BASE_URL}/api/admin/finance`, {
                headers: {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const statsRes = await fetch(`${API_BASE_URL}/api/admin/dashboard`, { headers });
                if (statsRes.ok) {
                ...
                const repRes = await fetch(`${API_BASE_URL}/api/admin/reports`, { headers });
                if (repRes.ok) {
                ...
                const actRes = await fetch(`${API_BASE_URL}/api/admin/activity`, { headers });
                if (actRes.ok) {
    ...
    return (
        <div className={styles.dashboard}>
            ...
            <div className={styles.statsGrid}>
                ...
                <div className={styles.statCard}>
                    <h3>Total Revenue</h3>
                    <p className={styles.number}>${(stats.totalRevenue || 0).toLocaleString()}</p>
                </div>
                <div className={styles.statCard}>
                    <h3>Active Skills</h3>
                    <p className={styles.number}>{stats.activeSkills || 0}</p>
                </div>
                <div className={styles.statCard}>
                    <h3>Total Bookings</h3>
                    <p className={styles.number}>{reports.bookingCount || 0}</p>
                </div>
            </div>
